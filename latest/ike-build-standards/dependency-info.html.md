---
date_published: 2026-08-06
date_modified: 2026-08-06
canonical_url: https://ike.network/ike-tooling/ike-build-standards/dependency-info.html
---

# Maven Coordinates

## [Apache Maven](#apache-maven)

```
<dependency>
  <groupId>network.ike.tooling</groupId>
  <artifactId>ike-build-standards</artifactId>
  <version>243</version>
  <type>pom</type>
</dependency>
```

## [Apache Ivy](#apache-ivy)

```
<dependency org="network.ike.tooling" name="ike-build-standards" rev="243">
  <artifact name="ike-build-standards" type="pom" />
</dependency>
```

## [Groovy Grape](#groovy-grape)

```
@Grapes(
@Grab(group='network.ike.tooling', module='ike-build-standards', version='243')
)
```

## [Gradle/Grails](#gradle-grails)

```
implementation 'network.ike.tooling:ike-build-standards:243'
```

## [Scala SBT](#scala-sbt)

```
libraryDependencies += "network.ike.tooling" % "ike-build-standards" % "243"
```

## [Leiningen](#leiningen)

```
[network.ike.tooling/ike-build-standards "243"]
```
