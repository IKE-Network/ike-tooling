...
- **When changing any Java file, verify its javadoc is complete before completing the task.** At the reactor root, run `mvn compile javadoc:jar -DskipTests -DfailOnError=false -DfailOnWarnings=false` and scan the output for `warning:` lines. Fix them in the same change — do not defer.
- The release preflight enforces this: `ike:release-publish` runs the same `mvn compile javadoc:jar` across the reactor (see issue #168) and hard-fails on any warning. `ike:release-draft` warns instead of failing so you can preview what would block publish. Fixing at edit time keeps the release path green.
