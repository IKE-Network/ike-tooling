...
The `ike-workspace-maven-plugin` is in the `ike-pipeline` reactor,
co-released with `ike-parent`. ike-pipeline consumes `ike-maven-plugin`
as an external build dependency at `${ike-tooling.version}`.
